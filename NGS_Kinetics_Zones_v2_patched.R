...
# ── 1b. FALLNUMMER DETECTION & DUPLICATE CHECK ───────────────
fall_col <- grep("fall", names(df_raw), ignore.case = TRUE, value = TRUE)
if (length(fall_col) == 0) {
  warning("No 'Fallnummer' column found. Check column names:")
  print(names(df_raw))
} else {
  fall_col <- fall_col[1]
  cat(sprintf("Fallnummer column detected: '%s'\n", fall_col))
  # Rename to standard label so META_DEF src = "Fallnummer" works
  if (fall_col != "Fallnummer") {
    df_raw <- df_raw |> rename(Fallnummer = all_of(fall_col))
    cat(sprintf("  → renamed to 'Fallnummer'\n"))
  }
  dups <- df_raw$Fallnummer[duplicated(df_raw$Fallnummer)]
  if (length(dups) == 0) {
    cat(sprintf("Uniqueness check: ALL %d Fallnummern are unique. No duplicates.\n", nrow(df_raw)))
  } else {
    cat(sprintf("WARNING: %d duplicate Fallnummer(n) detected:\n", length(dups)))
    print(dups)
  }
}
...
META_DEF <- tribble(
  ...
  "Fallnummer",     "Fallnummer",                                                       13,    "center",
...
n_unique <- length(unique(df_raw$Fallnummer))
cat(sprintf("Unique Fallnummern : %d  (%s)\n", n_unique,
            if (n_unique == nrow(df_raw)) "✓ all unique" else "⚠ DUPLICATES PRESENT"))
